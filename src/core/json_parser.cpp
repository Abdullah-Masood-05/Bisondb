  ...
  private:
    ...
    Value parseObject(int depth) {
        ...
        while (true) {
            ...
            if (kc == '"') {
                key = parseString();
            } else if (options_.relaxed && kc == '\'') {
                key = parseString('\'');
            } else if (options_.relaxed &&
                       (std::isalpha(static_cast<unsigned char>(kc)) || kc == '_' || kc == '$')) {
                key = parseIdentifierKey();
            } else {
    ...
    // Relaxed-mode identifier key: [A-Za-z_$][A-Za-z0-9_$]*.
    std::string parseIdentifierKey() {
        std::string out;
